...
Functions and Assets; `consent_form_file_name()`).
...
stage of this program, managing their proper inputs and clearing out any 
intermediate files.
...
asset `logs` to append each of them to that log file.
...
Scans through **NAME_FORM**, stopping whenever a whitespace character is 
reached to concatenate one of the function parameters (in the order of 
...
**NAME_EVENT** and replaces the first whitespace character in **NAME_FORM**.
...
The whitespace character is used as substitute for the variables in 
**NAME_FORM** due to it being the default delimiter for the C++ object 
`std::istringstream`.
...
identical, but those with mismatching whitespaces characters are not.
